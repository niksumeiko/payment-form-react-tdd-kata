...
const { getTestMethodName } = require('./utilities/jest.js');

const TEST_FUNCTION_NAMES = ['it'];
const VAGUE_TERMS = {
    should: /\bshould\b/i,
    correct: /correct/i,
...
function hasEmptyDescription({ arguments: args }) {
    return !args || !args[0] || (args[0].type !== 'Literal' && args[0].type !== 'TemplateLiteral');
}
...
function containsVagueWord(description, allowed) {
    return Object.entries(VAGUE_TERMS)
        .map(([word, regex]) => (allowed.includes(word) ? false : description.match(regex)))
        .filter((words) => words)
...
module.exports = {
    ...
    create(context) {
        ...
        function validate(node) {
            if (notTestFunction(node) || isIgnoredFunctionName(node) || hasEmptyDescription(node)) {
                return;
            }
            ...
            if (containingVagueWords.length) {
                const method = getTestMethodName(node);
                const vagueWords = commaListsAnd`${containingVagueWords}`;
                context.report({
